...
# SQLModel 데이터베이스 연결 설정
DATABASE_URL = f"postgresql://{PG_CONNECTION['user']}:{PG_CONNECTION['password']}@{PG_CONNECTION['host']}:{PG_CONNECTION['port']}/{PG_CONNECTION['database']}"
target_engine = create_engine(DATABASE_URL)

REFERENCE_PG_CONNECTION = {
    "host": "reborn-dev.c5ce2s0ospo9.ap-northeast-2.rds.amazonaws.com",
    "database": "postgres",
    "user": "reborn",
    "password": "Wlsdn2215!",
    "port": 5432,
...
REFERENCE_DATABASE_URL = f"postgresql://{REFERENCE_PG_CONNECTION['user']}:{REFERENCE_PG_CONNECTION['password']}@{REFERENCE_PG_CONNECTION['host']}:{REFERENCE_PG_CONNECTION['port']}/{REFERENCE_PG_CONNECTION['database']}"
...
def connect_to_reference_postgres():
    ...
    try:
        conn = psycopg2.connect(
            **REFERENCE_PG_CONNECTION, cursor_factory=RealDictCursor
        )
        return conn
...
def fetch_source_data() -> tuple[list[dict], list[dict], list[dict]]:
    """소스 테이블에서 데이터 가져오기"""
...
def parse_park_free_hours(hall_info: dict) -> int:
    """무료 주차 시간 파싱"""
